...
console.log = function(msg){
    ...
    handlers = {
        ...
        pairsElement: function(firstSpan, secondSpan) {
            ...
            var span2 = document.createElement("span");
            if (secondSpan[0] !== null) {
                if (secondSpan[0].customStyle) {
                    var el = document.createElement(secondSpan[0].customStyle.tagname)
                    if (secondSpan[0].customStyle.children) {
                        var el2 = document.createElement(secondSpan[0].customStyle.children[0].tagname)
                        el2.innerHTML = secondSpan[0].customStyle.children[0].content
                    }
                    el.append(el2);
                    span2.style.color = secondSpan[1]
                    span2.append(el)
        ...
        //clorsure function to aviod loop and events data lost
        event: function (obj, handlers) {
            function() {
                var arr = [];
                if (this.getElementsByTagName('div')[0] === undefined) {
                  for (var i of handlers.dom(obj)) {
                      i.style.paddingLeft = "1rem";
                      this.append(i);
                  }
                }
        ...
        details: function (object, prop) {
            var details = document.createElement("details");
            details.addEventListener("click", handlers.event(object, handlers));
            details.style.cursor = 'pointer'
            //details.style.color = "#008B8B"
            var summary = document.createElement("summary");
            summary.style["color"] = "indigo";
            var summarySpan1 = document.createElement("strong");
            summarySpan1.innerHTML = prop+" "
            var summarySpan = document.createElement("span")
            summarySpan.style["color"] = "#000";
            summarySpan.innerHTML = object.toString().replace(/object\s+/gm, "")
            summary.append(summarySpan1, summarySpan);
            details.append(summary);
            return details
        },
        dom: function dom(i) {
    ...
    function mainConsoleHighlighter(arg) {
        for(var i of arg) {
            if(typeof i === "object") {
                arr.push(handlers.details(i, ""))
            } else if(typeof i === "string") {
                arr.push(handlers.div("red", i))
            } else if(typeof i === "number") {
                arr.push(handlers.div("royalblue", i))
            } else if(typeof i === "function") {
                arr.push(handlers.div("", i))
            }
        }
    }
    mainConsoleHighlighter(arguments);
